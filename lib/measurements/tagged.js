...
TaggedMeasurement.prototype.prometheus = function(deltaTime, suffix) {
    const result = Array.from(this._prometheus.fields.values(), record => {
        ...
        if (record.field.single) {
            add(record.field.code + '_min', record.tags, record.field.min, ' (min)');
            add(record.field.code + '_max', record.tags, record.field.max, ' (max)');
            ...
            record.field.code.forEach((cur, index) => {
                ...
                add(code + '_min', record.tags, data.min[index], ' ' + help + ' (min)');
                add(code + '_max', record.tags, data.max[index], ' ' + help + ' (max)');
            });
        }
        return fields;
    }).filter(x => x).join('\n');
