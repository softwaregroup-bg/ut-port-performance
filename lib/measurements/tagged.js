...
TaggedMeasurement.prototype.register = function(type, code, name, interval) {
    ...
    return (id, tags, ...params) => {
        ...
        params.length && this.updatedFields.add(id);
...
TaggedMeasurement.prototype.influx = function(deltaTime, tags, suffix) {
    ...
    var response = Array.from(this.updatedFields.values(), (id) => {
        ...
        if (record.field.min != null) {
            fields += `min=${record.field.min},`;
        }
        if (record.field.max != null) {
            fields += `max=${record.field.max},`;
        }
        fields += `${record.field.code}=${record.field.flush(deltaTime)}`;
        return `${this.name}${this.influxTags}${record.influxTags}${externalTags} ${fields}${suffix}`;
    }).join('\n');
