...
TaggedMeasurement.prototype.counter = function(deltaTime, suffix) {
    const result = Array.from(this._prometheus.fields.values(), record => {
        ...
        record.field.getAvgCount && add('rate', record.name, record.field.getAvgCount(deltaTime), ' (rate)');
        ...
        if (record.field.single) {
            ...
            record.field.code.forEach((cur, index) => {
                const {code, help} = typeof cur === 'string' ? {code: cur, help: cur} : cur;
                add(code, record.name, changed ? data.value[index] : 0, ' ' + help);
                add(code + '_min', record.name, changed ? data.min[index] : 0, ' ' + help + ' (min)');
                add(code + '_max', record.name, changed ? data.max[index] : 0, ' ' + help + ' (max)');
            });
        }
        return fields;
    }).filter(x => x).join('\n');
