...
StandardMeasurement.prototype.counter = function(deltaTime, suffix) {
    const result = Array.from(this._prometheus.fields.values(), field => {
        const changed = field.changed;
        let result = '';
        if (field.min != null) result += `${this._prometheus.name}${this.name}_${field.code}_min ${changed ? field.min : 0}${suffix}\n`;
        if (field.max != null) result += `${this._prometheus.name}${this.name}_${field.code}_max ${changed ? field.max : 0}${suffix}\n`;
        return result + `${this._prometheus.name}${this.name}_${field.code} ${changed ? field.flush(deltaTime) : 0}${suffix}`;
    }).join('\n');
    return result;
};
