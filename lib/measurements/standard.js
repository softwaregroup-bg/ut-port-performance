...
StandardMeasurement.prototype.prometheus = function(deltaTime, suffix) {
    const result = Array.from(this._prometheus.fields.values(), field => {
        ...
        if (field.min != null) result += `${this.name}_${field.code}_min${this._prometheus.tags} ${field.min}${suffix}\n`;
        if (field.max != null) result += `${this.name}_${field.code}_max${this._prometheus.tags} ${field.max}${suffix}\n`;
        return result + `${this.name}_${field.code}${this._prometheus.tags} ${field.flush(deltaTime)}${suffix}`;
    }).join('\n');
